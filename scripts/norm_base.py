...
"""
Edit
Author: Stefan Setyadi Tjeng
Date: 26 August 2015
Note: base for further normalization because many language have different formatting
"""
import re
import os
import json
...
def cleanText(text, acronymFile):
    ...
    text = text.lower()
    # protect not identified (not in our list) abbrs and acronynms, by
    # replacing periods with underscore, A.B.C.D.E.F.=>A_B_C_D_E_F_
    text = re.sub('([a-z])\.([a-z])', '\g<1>_\g<2>', text)
    text = re.sub('_([a-z])\.', '_\g<1>_', text)
    text = re.sub('_([a-z])\.', '_\g<1>_', text)
    # Remove phrases enclosed in the brackers, eg: (word1 word 2), <word3
    # word4> and etc.
    text = re.sub('(\[.*?\]|\(.*?\)|\{.*?\}|<.*?>)', '', text)
    text = re.sub(' +', ' ', text)  # remove multiple spaces
    # Seperate floating numbers by 'point', eg: 1.4=> 1 point 4, .5  => 0
    # point 5
    ...
    # Put space between decimal and non decimal, eg: 50% => 50 %, 4am => 4 am,
    # 100$ => 100 $, #5 => # 5
    text = re.sub('(\d)(\D)', '\g<1> \g<2>', text)
    text = re.sub('(\D)(\d)', '\g<1> \g<2>', text)
    text = re.sub(' +', ' ', text)  # remove multiple spaces
    # Replace symbols, eg: 2 $=>2 dollars,1 $=>1 dollar, 5 %=>5 percents and
    # etc.
    ...
    text = re.sub('(\d) > (\d)', '\g<1> greater than \g<2>', text)
    ...
    # Replace the rest symbols with the period, in order to split sentences by
    # period later.
    text = re.sub('(!+|\?+|;+|:+)', '.', text)
    return text
